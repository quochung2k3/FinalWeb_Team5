                ...
                <c:forEach var="item" items="${listBill}">
                    <tr>
                        ...
                        <td style="
                                            ...
                                            align-items: center;
                                        ">
                            <a href="admin-bill-details?mahd=${item.maHD}" class="settings" data-toggle="tooltip">Xem
                                chi tiết</a>
                        </td>
                    </tr>
                </c:forEach>
                    ...
                    <c:choose>
                        <c:when test="${not empty maHoaDon or not empty maChiNhanh}">
                            <li class="page-item"><a id="linkPagging${1}" class="page-link active"
                                                     ...
                                                     href="admin-bill-search?index=1&maChiNhanh=${param.maChiNhanh}&maHoaDon=${param.maHoaDon}" ${index==1 ? "style=\"color: red;\"" : ""}>1</a>
                            </li>
                            <c:forEach begin="2" end="${numpage}" var="i">
                                <li class="page-item"><a id="linkPagging${i}" class="page-link"
                                                         ...
                                                         href="admin-bill-search?index=${i}&maChiNhanh=${param.maChiNhanh}&maHoaDon=${param.maHoaDon}" ${index==i ? "style=\"color: red;\"" : ""}>${i}</a>
                                </li>
                            </c:forEach>
                        </c:when>
                        <c:otherwise>
                            <li class="page-item"><a id="linkPagging${1}" class="page-link active"
                                                     onclick="searchAndUpdateTableByPaging(event, 1)"
                                                     href="admin-ql-bill?index=1">1</a></li>
                            <c:forEach begin="2" end="${numpage}" var="i">
                                <li class="page-item"><a id="linkPagging${i}" class="page-link"
                                                         onclick="searchAndUpdateTableByPaging(event, ${i})"
                                                         href="admin-ql-bill?index=${i}">${i}</a></li>
                            </c:forEach>
                        </c:otherwise>
                    </c:choose>
                </ul>
