                ...
                <c:forEach var="item" items="${listCustomer}">
                    <tr>
                        ...
                        <td style="
                                            ...
                                            align-items: center;
                                        ">
                            <a href="admin-customer-update?maKh=${item.maKh}" class="settings" title="Settings"
                               data-toggle="tooltip"><i class="material-icons">&#xE8B8;</i></a>
                            <a href="admin-customer-history?maKh=${item.maKh}" class="settings" title="Purchase History"
                               data-toggle="tooltip"><i class="material-icons">&#xE86A;</i></a>
                        </td>
                    </tr>
                </c:forEach>
                    ...
                    <c:choose>
                        <c:when test="${not empty name or not empty total}">
                            <li class="page-item"><a id="linkPagging${1}" class="page-link active"
                                                     ...
                                                     href="admin-customer-search?index=1&name=${param.name}&total=${param.total}" ${index==1 ? "style=\"color: red;\"" : ""}>1</a>
                            </li>
                            <c:forEach begin="2" end="${numpage}" var="i">
                                <li class="page-item"><a id="linkPagging${i}" class="page-link"
                                                         ...
                                                         href="admin-customer-search?index=${i}&name=${param.name}&total=${param.total}" ${index==i ? "style=\"color: red;\"" : ""}>${i}</a>
                                </li>
                            </c:forEach>
                        </c:when>
                        <c:otherwise>
                            <li class="page-item"><a id="linkPagging${1}" class="page-link active"
                                                     onclick="searchAndUpdateTableByPaging(event, 1)"
                                                     href="admin-ql-customer?index=1">1</a></li>
                            <c:forEach begin="2" end="${numpage}" var="i">
                                <li class="page-item"><a id="linkPagging${i}" class="page-link"
                                                         onclick="searchAndUpdateTableByPaging(event, ${i})"
                                                         href="admin-ql-customer?index=${i}">${i}</a></li>
                            </c:forEach>
                        </c:otherwise>
                    </c:choose>
                </ul>
