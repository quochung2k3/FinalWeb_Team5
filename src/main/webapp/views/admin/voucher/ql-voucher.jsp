        ...
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
                    ...
                    <div class="col-sm-8">
                        <a href="<c:url value="/admin-voucher-add"/>"
                           class="btn btn-primary"><span>Add Voucher</span></a>
                ...
                <c:forEach var="item" items="${listVoucher}">
                    <tr>
                        ...
                        <td style="
                                            ...
                                            align-items: center;
                                        ">
                            <a href="admin-voucher-update?maVC=${item.maVC}" class="settings" title="Settings"
                               data-toggle="tooltip"><i class="material-icons">&#xE8B8;</i></a>
                            <a href="admin-voucher-delete?maVC=${item.maVC}" class="delete trigger-btn" title="Delete"
                               data-item-id="${item.maVC}" onclick="openModal(event)"><i
                    ...
                    <c:choose>
                        <c:when test="${not empty maVC or not empty status}">
                            <li class="page-item"><a id="linkPagging${1}" class="page-link active"
                                                     ...
                                                     href="admin-voucher-search?index=1&maVC=${param.maVC}&status=${param.status}" ${index==1 ? "style=\"color: red;\"" : ""}>1</a>
                            </li>
                            <c:forEach begin="2" end="${numpage}" var="i">
                                <li class="page-item"><a id="linkPagging${i}" class="page-link"
                                                         ...
                                                         href="admin-voucher-search?index=${i}&maVC=${param.maVC}&status=${param.status}" ${index==i ? "style=\"color: red;\"" : ""}>${i}</a>
                                </li>
                            </c:forEach>
                        </c:when>
                        <c:otherwise>
                            <li class="page-item"><a id="linkPagging${1}" class="page-link active"
                                                     onclick="searchAndUpdateTableByPaging(event, 1)"
                                                     href="admin-ql-voucher?index=1">1</a></li>
                            <c:forEach begin="2" end="${numpage}" var="i">
                                <li class="page-item"><a id="linkPagging${i}" class="page-link"
                                                         onclick="searchAndUpdateTableByPaging(event, ${i})"
                                                         href="admin-ql-voucher?index=${i}">${i}</a></li>
                            </c:forEach>
                        </c:otherwise>
                    </c:choose>
                </ul>
